...
**Custom**을 선택하거나 `ocx init`에서 `custom`을 선택한 뒤 베이스 URL을 입력하세요. 호출자
`User-Agent`를 업스트림으로 보내려면 `forwardUserAgent: true`를 설정하세요(기본 끔). 모든 프로바이더 필드
(`headers`, `forwardUserAgent`, `noReasoningModels`, `noVisionModels`, `models`, …)는
