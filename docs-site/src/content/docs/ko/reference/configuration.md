...
| `forwardUserAgent?` | `boolean` | `true`이면 `openai-chat` / `openai-responses`(key 모드)가 호출자 `User-Agent`를 업스트림으로 복사합니다. 기본은 끔(Bun `fetch` UA). 정적 `headers`의 `User-Agent`가 우선합니다. |
