...
| `forwardUserAgent?` | `boolean` | `true`이면 커스텀 채널 어댑터(`openai-chat` / `openai-responses` / `anthropic` / `google` / `azure-openai`)가 호출자 `User-Agent`를 업스트림으로 복사합니다. 기본은 끔(런타임/프로토콜 UA). 정적 `headers`의 `User-Agent`가 우선합니다. |
