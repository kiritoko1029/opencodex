...
| `forwardUserAgent?` | `boolean` | `true` のとき、カスタムチャネル adapter（`openai-chat` / `openai-responses` / `anthropic` / `google` / `azure-openai`）が呼び出し元の `User-Agent` を上流へコピーします。デフォルトはオフ（ランタイム/プロトコル UA）。静的 `headers` の `User-Agent` が優先します。 |
